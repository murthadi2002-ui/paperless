...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, onAddClick, breadcrumbs = [] }) => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-cairo transition-all duration-300 text-right" dir="rtl">
      {/* Sidebar */}
      <aside 
        className={`${isCollapsed ? 'w-20' : 'w-64'} bg-white border-l border-slate-200 flex flex-col transition-all duration-300 z-40 relative group/sidebar shadow-xl shadow-slate-200/50`}
      >
        <button 
        ...
        <div className={`p-6 flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'} transition-all`}>
          <div className="bg-emerald-600 text-white p-2.5 rounded-2xl shrink-0 shadow-lg shadow-emerald-100">
            <PlusCircle size={isCollapsed ? 20 : 24} />
          </div>
          {!isCollapsed && (
            <h1 className="text-2xl font-black text-slate-800 tracking-tight animate-in fade-in slide-in-from-right-2">Paperless</h1>
          )}
        </div>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center ${isCollapsed ? 'justify-center' : 'gap-3 px-4'} py-3.5 rounded-2xl text-sm font-black transition-all duration-200 relative group ${
                  activeTab === item.id 
                    ? 'bg-emerald-50 text-emerald-700 shadow-sm' 
                    : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'
                }`}
              >
                <Icon size={20} className={`${activeTab === item.id ? 'text-emerald-600' : ''} shrink-0 transition-transform group-hover:scale-110`} />
        ...
        <div className="p-3 border-t border-slate-100">
          <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3 p-3 bg-slate-50 rounded-2xl'} transition-all`}>
            <img src={CURRENT_USER.avatar} alt="User" className={`w-10 h-10 rounded-2xl border-2 border-white shadow-sm shrink-0 object-cover ${isCollapsed ? 'w-8 h-8' : ''}`} />
            {!isCollapsed && (
              <>
                <div className="flex-1 overflow-hidden text-right">
                  <p className="text-xs font-black text-slate-800 truncate leading-none">{CURRENT_USER.name}</p>
                  <p className="text-[10px] font-bold text-slate-400 truncate mt-1">{CURRENT_USER.email}</p>
                </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className="h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 px-8 flex items-center justify-between z-30 shrink-0 sticky top-0">
          <div className="flex items-center gap-2">
            <span className="text-slate-400 text-[10px] font-black uppercase tracking-widest hidden sm:block">نظام الأرشفة الموحد</span>
            <ChevronLeft size={14} className="text-slate-200 hidden sm:block" />
            <div className="flex items-center gap-2">
              <button className="text-slate-800 font-black text-sm">{menuItems.find(m => m.id === activeTab)?.label}</button>
              {breadcrumbs.map((crumb, idx) => (
                <React.Fragment key={idx}>
                  <ChevronLeft size={14} className="text-slate-300" />
                  <span className="text-sm font-black text-emerald-600 truncate max-w-[150px]">{crumb.label}</span>
                </React.Fragment>
              ))}
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 relative">
            <div className="relative" ref={notificationRef}>
              <button 
                ...
                className={`relative p-2.5 rounded-2xl transition-all ${isNotificationsOpen ? 'bg-emerald-50 text-emerald-600' : 'text-slate-400 hover:text-emerald-600 hover:bg-slate-50'}`}
              >
                <Bell size={22} />
                {unreadCount > 0 && (
                  /* Red Dot Indicator - Minimalist Nazk Design */
                  <span className="absolute top-2.5 right-2.5 w-2 h-2 bg-red-500 rounded-full border border-white shadow-sm animate-pulse"></span>
                )}
              </button>

              {/* NOTIFICATION DROPDOWN - NAZK STYLE */}
              {isNotificationsOpen && (
            ...
            <button onClick={onAddClick} className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-2xl text-xs font-black flex items-center gap-2 transition-all shadow-lg shadow-emerald-100 active:scale-95">
              <PlusCircle size={18} />
