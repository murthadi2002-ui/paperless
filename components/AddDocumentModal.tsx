...
const AddDocumentModal: React.FC<AddDocumentModalProps> = ({ 
  ...
  }, [isOpen, defaultProjectId, defaultFolderId, initialAttachments]);
      ...
      <div className="bg-white w-full max-w-2xl max-h-[85vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-300 border border-slate-200">
        <div className="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[8px] text-slate-400 font-bold uppercase tracking-wider">{initialAttachments.length > 0 ? 'استيراد من المراسلات' : 'سجل إداري ذكي'}</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
          ...
          <div className="space-y-4">
            <div onClick={() => fileInputRef.current?.click()} className={`border-2 border-dashed rounded-xl p-4 flex flex-col items-center justify-center bg-slate-50/50 transition-all cursor-pointer group shadow-inner border-slate-200 hover:bg-emerald-50/50 hover:border-emerald-300`}>
              <input type="file" multiple ref={fileInputRef} className="hidden" onChange={handleFileUpload} />
              <Upload size={20} className="text-slate-300 transition-all mb-2 group-hover:text-emerald-500" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {uploadedFiles.map((f) => (
                  <div key={f.id} className="bg-white p-2.5 rounded-xl border border-slate-100 flex items-center gap-2 group shadow-sm">
                    <div className="p-1.5 rounded-lg shrink-0 bg-emerald-50 text-emerald-600">
                      <FileText size={14} />
                    </div>
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-end shrink-0 gap-2">
          ...
          <button onClick={handleSubmit} disabled={!isFormValid || processing} className="px-8 py-2 text-white bg-emerald-600 rounded-lg text-[10px] font-black shadow-lg disabled:opacity-50 transition-all flex items-center gap-2 active:scale-95 hover:bg-emerald-700 shadow-emerald-100">
