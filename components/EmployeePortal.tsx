...
const EmployeePortal: React.FC<EmployeePortalProps> = ({ documents, onOpenDoc }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 text-right" dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 items-start">
        {myTasks.length > 0 ? (
          myTasks.map(({ doc, task }) => {
            ...
            const otherAssignees = MOCK_EMPLOYEES.filter(e => task.assigneeIds.includes(e.id) && e.id !== CURRENT_USER.id);
            ...
            return (
              <div key={task.id} onClick={() => setSelectedTask({doc, task})} className={`bg-white p-6 rounded-[2.5rem] border transition-all group cursor-pointer overflow-hidden flex flex-col relative ${selectedTask?.task.id === task.id ? 'ring-4 ring-amber-500/10 border-amber-500 shadow-2xl scale-[1.02]' : 'border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-1'}`}>
                {/* Meta Header - From/To Design */}
                <div className="flex items-center justify-between mb-5 bg-slate-50 p-3 rounded-2xl border border-slate-100/50">
                  <div className="flex items-center gap-2">
                    <img src={issuer?.avatar} className="w-8 h-8 rounded-xl object-cover border-2 border-white shadow-sm" alt="" />
                    <div className="text-right">
                      <p className="text-[8px] font-black text-slate-400 uppercase leading-none">مِن (المُوجّه)</p>
                      <p className="text-[10px] font-black text-indigo-600 truncate">{issuer?.name.split(' ')[0]}</p>
                    </div>
                  </div>
                  <div className="h-6 w-[1px] bg-slate-200"></div>
                  <div className="flex items-center gap-2">
                    <div className="text-left">
                      <p className="text-[8px] font-black text-slate-400 uppercase leading-none">إلى (المكلفون)</p>
                      <p className="text-[10px] font-black text-amber-600">أنت {otherAssignees.length > 0 ? `+${otherAssignees.length}` : ''}</p>
                    </div>
                    <div className="flex -space-x-1 flex-row-reverse">
                      <img src={CURRENT_USER.avatar} className="w-8 h-8 rounded-xl object-cover border-2 border-white shadow-sm ring-1 ring-amber-100" alt="" />
                      {otherAssignees.slice(0, 2).map(a => (
                        <img key={a.id} src={a.avatar} className="w-8 h-8 rounded-xl object-cover border-2 border-white shadow-sm" alt="" />
                      ))}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-1 space-y-2 mb-4">
                  ...
                  <div className="flex items-center gap-3">
                     <span className="text-[9px] font-black text-slate-400 bg-white border border-slate-100 px-2.5 py-1 rounded-lg">المرجع: {doc.refNumber}</span>
                     <div className="flex items-center gap-1.5 text-[10px] font-black text-slate-800">
                        <Clock size={12} className={task.status === 'completed' ? 'text-emerald-500' : 'text-amber-500'} />
                        {task.dueDate}
                     </div>
                  </div>
                </div>

                <div className="p-4 bg-slate-50/80 rounded-[1.8rem] border border-slate-100 mb-6 group-hover:bg-white transition-all shadow-inner">
                  <p className="text-[11px] font-bold text-slate-600 leading-relaxed italic line-clamp-2">"{task.instructions}"</p>
                </div>
      ...
      {/* Task Response Sidebar Overlay - Updated Header */}
      {selectedTask && (
        <div className="fixed inset-0 z-[100] flex justify-end animate-in fade-in duration-300">
           ...
           <div className="relative w-full max-w-xl bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-left duration-500 ease-out border-r border-slate-200">
              <div className="p-8 border-b border-slate-100 flex items-center justify-between shrink-0 bg-white/80 backdrop-blur-md sticky top-0 z-10">
                 <div className="flex items-center gap-4">
                    ...
                    <div>
                       ...
                       <p className="text-[10px] text-amber-600 font-black uppercase tracking-widest">توجيه صادر من: {MOCK_EMPLOYEES.find(e => e.id === selectedTask.task.issuerId)?.name}</p>
                    </div>
                 </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-10 space-y-10 custom-scrollbar">
                 ...
                 {/* Direction Metadata (Issuer & Others) */}
                 <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-3">
                       <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-widest pr-1">مُصدر التوجيه</h5>
                       <div className="flex items-center gap-3 p-4 bg-indigo-50/50 border border-indigo-100 rounded-2xl">
                          <img src={MOCK_EMPLOYEES.find(e => e.id === selectedTask.task.issuerId)?.avatar} className="w-10 h-10 rounded-xl object-cover border-2 border-white shadow-sm" alt="" />
                          <div>
                             <p className="text-xs font-black text-slate-800">{MOCK_EMPLOYEES.find(e => e.id === selectedTask.task.issuerId)?.name}</p>
                             <p className="text-[9px] font-bold text-indigo-600">{MOCK_EMPLOYEES.find(e => e.id === selectedTask.task.issuerId)?.department}</p>
                          </div>
                       </div>
                    </div>
                    <div className="space-y-3">
                       <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-widest pr-1">الزملاء المكلفون</h5>
                       <div className="flex flex-wrap gap-2 p-4 bg-slate-50 border border-slate-100 rounded-2xl min-h-[74px] items-center">
                          {MOCK_EMPLOYEES.filter(e => selectedTask.task.assigneeIds.includes(e.id)).map(a => (
                            <img key={a.id} src={a.avatar} className="w-8 h-8 rounded-lg object-cover border-2 border-white shadow-sm" title={a.name} alt="" />
                          ))}
                       </div>
                    </div>
                 </div>
