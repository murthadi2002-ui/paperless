  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 text-right" dir="rtl">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h2 className="text-2xl font-black text-slate-800">بوابة المتابعة الذكية</h2>
          <p className="text-slate-400 text-xs mt-1 font-bold">إدارة المراسلات، التوجيهات، والمهام الإدارية اللحظية</p>
        </div>
        <div className="flex gap-2 p-1.5 bg-white border border-slate-200 rounded-2xl shadow-sm h-fit">
           <button onClick={() => setActiveTab('incoming')} className={`px-5 py-2 rounded-xl text-[10px] font-black transition-all flex items-center gap-2 ${activeTab === 'incoming' ? 'bg-amber-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}>
             <ClipboardList size={14} /> مهامي (الواردة)
           </button>
           <button onClick={() => setActiveTab('outgoing')} className={`px-5 py-2 rounded-xl text-[10px] font-black transition-all flex items-center gap-2 ${activeTab === 'outgoing' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}>
             <SendHorizontal size={14} /> توجيهاتي (الصادرة)
           </button>
           <button onClick={() => setActiveTab('completed')} className={`px-5 py-2 rounded-xl text-[10px] font-black transition-all flex items-center gap-2 ${activeTab === 'completed' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 items-start">
        {filteredTasks.length > 0 ? (
          filteredTasks.map(({ doc, task }) => {
            ...
            return (
              <div key={task.id} onClick={() => setSelectedTask({doc, task})} className={`bg-white p-6 rounded-[2.5rem] border transition-all group cursor-pointer overflow-hidden flex flex-col relative ${selectedTask?.task.id === task.id ? 'ring-4 ring-emerald-500/10 border-emerald-500 shadow-2xl scale-[1.01]' : 'border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1'}`}>
                {/* Meta Labels */}
                ...
                {/* Refined Identity Section with Enhanced Names */}
                <div className="flex flex-col gap-2.5 mb-6 bg-slate-50/50 p-4 rounded-[1.8rem] border border-slate-100/50">
                   {/* Full Issuer Capsule */}
                ...
                <div className="flex-1 space-y-2 mb-5">
                  <h4 className="text-[13px] font-black text-slate-800 leading-snug line-clamp-2">{doc.subject}</h4>
                ...
                <div className="p-4 bg-slate-50/80 rounded-2xl border border-slate-100 mb-6 italic">
          ...
          <div className="col-span-full py-24 text-center bg-white rounded-[3rem] border-2 border-dashed border-slate-100 animate-in fade-in zoom-in-95">
             ...
             <p className="text-slate-400 text-sm font-bold mt-2">لا توجد توجيهات {activeTab === 'outgoing' ? 'صادرة منك' : 'واردة إليك'} في الوقت الحالي</p>
          </div>
        )}
      </div>

      {/* Task Response Sidebar Overlay - Re-refined */}
      {selectedTask && (
           ...
           <div className="relative w-full max-w-xl bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-left duration-500 ease-out border-r border-slate-200">
              <div className="p-6 px-8 border-b border-slate-100 flex items-center justify-between shrink-0 bg-white/80 backdrop-blur-md sticky top-0 z-10">
                 <div className="flex items-center gap-4">
                    <button onClick={() => setSelectedTask(null)} className="p-2 hover:bg-slate-100 rounded-2xl text-slate-300 transition-all"><X size={20} /></button>
                    <div className="h-6 w-[1px] bg-slate-100"></div>
                    <div>
                       <h3 className="text-base font-black text-slate-900">سجل التوجيه الإداري</h3>
                       <p className="text-[9px] text-emerald-600 font-black uppercase tracking-widest">معاينة تفاصيل التنفيذ</p>
                    </div>
                 </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar bg-slate-50/20">
                 {/* Document Info Card */}
                 <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm ring-1 ring-slate-100/50">
                    <div className="flex items-center gap-4 mb-6">
                       <div className="p-3 bg-emerald-50 text-emerald-600 rounded-2xl shadow-sm"><FileText size={24} /></div>
                       <div className="overflow-hidden text-right">
                          <h4 className="text-[13px] font-black text-slate-800 leading-snug">{selectedTask.doc.subject}</h4>
                          <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase">مرجع: {selectedTask.doc.refNumber}</p>
                       </div>
                    </div>
                    <button onClick={() => onOpenDoc(selectedTask.doc)} className="w-full py-3 bg-slate-900 text-white rounded-2xl text-[10px] font-black hover:bg-slate-800 transition-all shadow-xl shadow-slate-200 flex items-center justify-center gap-2">استعراض ملف الوثيقة الأصلي <ArrowUpRight size={14}/></button>
                 </div>

                 {/* Flow Section with Enhanced Full Issuer Pill */}
                 <div className="space-y-6">
                    <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] pr-2">أطراف التكليف الإداري</h5>
                    
                    <div className="flex flex-col gap-8 p-8 bg-white rounded-[3rem] border border-slate-100 shadow-sm relative overflow-hidden">
                       ...
                       {/* Connection Line */}
                       <div className="absolute top-[90px] right-[64px] bottom-[115px] w-0.5 bg-slate-100/60 z-0"></div>
                       
                       {/* Enhanced Issuer Capsule with Name and Photo */}
                       <div className="flex items-center gap-4 relative z-10">
                          <div className="flex items-center gap-4 bg-white px-5 py-3.5 rounded-full border border-indigo-100 shadow-md ring-4 ring-indigo-50/50">
                       ...
                       <div className="flex flex-col gap-3 relative z-10 mr-12">
                          ...
                          <div className="flex flex-wrap gap-2.5">
                             {MOCK_EMPLOYEES.filter(e => selectedTask.task.assigneeIds.includes(e.id)).map(a => (
                               <div key={a.id} className={`flex items-center gap-2.5 px-3.5 py-2.5 rounded-full border shadow-sm transition-all ${a.id === CURRENT_USER.id ? 'bg-amber-600 border-amber-600 text-white' : 'bg-white border-slate-100 text-slate-600 hover:border-amber-200'}`}>
                 ...
                 <div className="space-y-4">
                    ...
                    <div className="p-8 bg-white border border-amber-100 rounded-[2.5rem] relative overflow-hidden shadow-sm">
                       <div className="absolute top-0 right-0 w-1.5 h-full bg-amber-400/20"></div>
                 ...
                 {selectedTask.task.status === 'pending' ? (
                   <div className="space-y-4 pt-4">
                      {selectedTask.task.issuerId === CURRENT_USER.id ? (
                        <div className="p-8 bg-indigo-50 border border-indigo-100 rounded-[2.5rem] flex flex-col items-center text-center gap-4">
                           <div className="p-4 bg-white rounded-full shadow-lg text-indigo-400 ring-8 ring-indigo-50">
                             <Clock size={40} />
                           </div>
                           <div>
                        ...
                        <>
                          ...
                          <textarea 
                            ...
                            className="w-full px-6 py-5 bg-white border border-slate-200 rounded-[2.5rem] outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all font-bold text-sm resize-none h-40 text-right shadow-sm"
                          />
                          <button disabled={!response.trim()} className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-black text-sm shadow-xl shadow-emerald-100 hover:bg-emerald-700 transition-all flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50">
