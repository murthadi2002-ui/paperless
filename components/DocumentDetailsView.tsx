...
const DocumentDetailsView: React.FC<DocumentDetailsViewProps> = ({ 
      ...
      setDownloadProgress(prev => {
        if (prev >= 100) {
          ...
          doc.attachments.forEach((at, idx) => {
            setTimeout(() => triggerRealDownload(at), idx * 400);
          });
          setTimeout(() => setDownloadingId(null), 1000);
          return 100;
        }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 text-right" dir="rtl" onClick={() => setIsMenuOpen(false)}>
      ...
      {/* Header - Changed from rounded-[2.5rem] to rounded-2xl */}
      <div className="flex flex-col lg:flex-row items-center justify-between bg-white p-4 px-6 rounded-2xl border border-slate-200 shadow-sm gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0 w-full lg:w-auto justify-center lg:justify-end">
          <button 
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black hover:bg-white hover:shadow-xl transition-all border border-slate-200"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        {/* Sidebar Info - Changed from rounded-[3rem] to rounded-2xl */}
            ...
            <div className="space-y-4 max-h-[450px] overflow-y-auto px-1 custom-scrollbar">
               {doc.tasks && doc.tasks.length > 0 ? (
                 doc.tasks.map((task, idx) => {
                   ...
                   return (
                     <div key={task.id} className="bg-slate-50/50 p-6 rounded-xl border border-slate-100 space-y-5 shadow-sm">
                        ...
                        <p className="text-[11px] font-bold text-slate-700 italic bg-white p-4 rounded-xl border border-slate-100 leading-relaxed shadow-sm">"{task.instructions}"</p>
                     </div>
                   );
                 })
               ) : (
        ...
        {/* Attachments Section - Changed from rounded-[3rem] to rounded-2xl */}
          ...
          <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {doc.attachments.map((file) => (
                  <div key={file.id} className="group bg-slate-50/50 p-5 rounded-xl border border-slate-100 hover:bg-white hover:shadow-xl hover:border-emerald-100 transition-all flex items-center justify-between gap-4">
                     ...
                     {/* Control buttons - Removed opacity-0 and group-hover classes */}
                     <div className="flex items-center gap-1 transition-all shrink-0">
                        ...
                        <button onClick={() => onDeleteAttachment(file.id)} className="p-2.5 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all" title="حذف"><Trash2 size={18} /></button>
                     </div>
                  </div>
                ))}
      ...
      {/* Task Modal - Changed from rounded-[2.8rem] to rounded-2xl */}
      {showTaskForm && (
           ...
           <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
              ...
              <div className="p-8 space-y-6">
                 <div className="space-y-1.5">
                    ...
                    <select 
                      multiple 
                      className="w-full h-32 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-xs custom-scrollbar"
                 ...
                 <div className="space-y-1.5">
                    ...
                    <input type="date" className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-xs" value={taskData.dueDate} onChange={e => setTaskData({...taskData, dueDate: e.target.value})} />
                 </div>
                 <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pr-2">التعليمات الفنية والإدارية</label>
                    <textarea className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-xs resize-none" rows={3} placeholder="اكتب التعليمات هنا..." value={taskData.instructions} onChange={e => setTaskData({...taskData, instructions: e.target.value})} />
                 </div>
                 <div className="flex gap-3 pt-4">
                    <button onClick={handleAddTask} className="flex-1 py-4 bg-emerald-600 text-white rounded-xl font-black text-sm shadow-xl shadow-emerald-100 hover:bg-emerald-700 transition-all">إرسال التوجيه</button>
                    <button onClick={() => setShowTaskForm(false)} className="px-8 py-4 bg-slate-100 text-slate-500 rounded-xl font-black text-sm">إلغاء</button>
                 </div>
              </div>
           </div>
        </div>
